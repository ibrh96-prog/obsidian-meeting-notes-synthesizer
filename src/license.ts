...
// Offline Ed25519 license verification for Meeting Notes Synthesizer.
...
const PUBLIC_KEY_B64 = "5v9+iN476NV+UhqI4uWS1y5d34gx5tcMvHimo4pNGjc=";
const PRODUCT_ID = "meeting-notes-synthesizer";
...
export function verifyLicense(licenseKey: string): LicenseStatus {
  ...
  try {
    ...
    const signatureOk = nacl.sign.detached.verify(payloadBytes, signature, publicKey);
    if (!signatureOk) {
